...
class Login extends DBConnection {
	...
	public function clogin(){
		...
		$qry = $this->conn->query("SELECT * from customers where  username = '$username' and `password` = '".$password."' ");
		if($qry->num_rows > 0){
			foreach($qry->fetch_array() as $k => $v){
				if(!is_numeric($k)){
					$this->settings->set_userdata($k,$v);
	...
	public function slogin(){
		...
		$qry = $this->conn->query("SELECT * from people where  email = '$email' and `password` =  '$password' ");
		if($qry->num_rows > 0){
			foreach($qry->fetch_array() as $k => $v){
				if(!is_numeric($k)){
					$this->settings->set_userdata($k,$v);
